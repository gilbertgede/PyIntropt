...
def sqp_solver(x0, f, c, f_x, c_x, H, x_l, x_u, c_l, c_u):
    ...
    nu_k = qp_nu             # simple bounds
    lambda_k = qp_lambda     # constraint bounds
    ...
    while True:
        ...
        if k > 0:
            ...
            # check constraints
            # check lambda signs
            term1 = ((asarray(c_l > c_k) * abs(asarray(c_l - c_k))).max() <= eps_d * eps
                 and (asarray(c_u < c_k) * abs(asarray(c_u - c_k))).max() <= eps_d * eps
                 and (asarray(x_l > x) * abs(asarray(x_l - x))).max() <= eps_d * eps
                 and (asarray(x_u < x) * abs(asarray(x_u - x))).max() <= eps_d * eps)
        ...
        while (min(eigsh(H_k, k=6, which='SA', maxiter=100000,
